...
function App() {
  ...
  const addItem = () => {
    ...
    const existingItem = dataItems.find((dataItem) => dataItem.item === item.name && dataItem.ppu === ppuRef.current.value);

    if (existingItem) {
      existingItem.qty = Number(existingItem.qty) + Number(qtyRef.current.value);
      existingItem.dis = Number(existingItem.dis) + Number(discountRef.current.value);
        ...
        <Col md={4} style={{ backgroundColor: "#e4e4e4" }}>
          ...
          <Row>
            <Col>
              ...
              <Form.Control type="number" ref={discountRef} defaultValue={1} />
            </Col>
          </Row>
