...
function QuotationTable({ data, clearDataItems, deleteByIndex }) {
  ...
  // const discount = v.dis;
  const total = data.reduce((acc, v) => acc + ((v.qty * v.ppu) - v.dis), 0);
  const total_discount = Math.abs(data.reduce((acc, v) => (acc - v.dis), 0));
      ...
      <Table striped bordered hover>
          ...
          data.map((v, i) => {
            ...
            return (
              <tr key={i}>
                ...
                <td className={style.textRight}>{discount}</td>
        ...
        <tfoot>
          <tr>
            ...
            <td className={style.textRight}>{total_discount}</td>
          </tr>
          <tr>
            <td colSpan={3} className={style.textRight}>
              Total
